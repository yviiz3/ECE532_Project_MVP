...
module bram_dualport_driver (
    ...
    //4 brams. 
     (* ram_style = "block" *)
    localparam mem_width = 131072; // 24576 words per BRAM
    reg [31:0] mem0 [0:mem_width-1];                                    

    always @(posedge clka) begin
        if (a_en) begin
            if (a_we) begin
                mem0[a_addr] <= a_din;
                a_dout <= a_din;   // write-first behavior
            end else begin
                a_dout <= mem0[a_addr];
            end
        end
    ...
    always @(posedge clkb) begin
        if (b_we) begin
            mem0[b_addr] <= b_din;
            b_dout <= b_din;   // write-first behavior
        end else begin
            b_dout <= mem0[b_addr];
        end
    end
...
module bram_mux(
    ...
    always @(posedge clk) begin
        status_array <= {status_4, status_3, status_2, status_1};
    end
