...
module svd_preloader_250_5dsp #(
    ...
    parameter int IDX_U = 1,
    parameter int IDX_VT = IDX_U + (N * N) + N
) (
    input  logic clk,
    input  logic rst,
    input  logic enable,
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            exp_v_bram_addr <= IDX_VT[EXP_ADDR_W-1:0];
            ...
            if (start_pixel) begin
                ...
                if ((!load_active) && (next_load_t < K) && (!target_bank_ready)) begin
                    ...
                    if (ld_issue_idx < BUF) begin
                        ...
                        exp_v_bram_addr <= IDX_VT + ((ld_base_t + ld_issue_idx) * N) + j;
