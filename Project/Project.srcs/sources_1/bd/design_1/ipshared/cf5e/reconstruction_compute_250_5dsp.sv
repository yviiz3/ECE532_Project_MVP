...
module reconstruction_compute_250_5dsp #(
    parameter int W = 16,
    ...
    parameter int K = 100,
    parameter int BUF = 5,
    parameter int EXP_MEM_DEPTH = 187751,
    ...
    // S is stored as an unsigned 16-bit integer in memory.
    ...
    logic signed [W-1:0] vt_buf_bank [0:1][0:BUF-1];
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            s_bram_addr <= IDX_S[EXP_ADDR_W-1:0];
            ...
            for (lane_ff = 0; lane_ff < BUF; lane_ff = lane_ff + 1) begin
                ...
                vt_buf_bank[0][lane_ff] <= '0;
                vt_buf_bank[1][lane_ff] <= '0;
                sv_buf[lane_ff] <= '0;
            end
        end else begin
            out_bram_we <= 1'b0;
            ...
            if (pre_wr_en) begin
                ...
                vt_buf_bank[pre_wr_bank][pre_wr_lane] <= pre_wr_v_data;
            end
